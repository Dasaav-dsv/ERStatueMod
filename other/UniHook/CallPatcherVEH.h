#ifndef UNIHOOK_CALLPATCHER_H
#define UNIHOOK_CALLPATCHER_H
	...
	public:
		...
		bool patchCall(void* callAddress, void* tgtAddress, uint8_t firstByte, int immOffset) {
			...
			if (std::abs(disp) > INT_MAX) {
				Logger::log(Logger::ERR, "When patching call at %p target address %p was out of range", callAddress, tgtAddress);
				return false;
			}
			...
			if (!CallPatcher::write(callAddress, static_cast<uint8_t>(0xCC))) {
				Logger::log(Logger::ERR, "When patching call at %p memory was inaccessible", callAddress);
