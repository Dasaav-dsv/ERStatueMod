#ifndef UNIHOOK_VFTHOOK_H
#define UNIHOOK_VFTHOOK_H
...
/// </summary>
template <typename HookType> class VFTHookTemplate : public VFTHookBase {
public:
	/// <summary>
	...
	template <typename F> VFTHookTemplate(const char* className, unsigned int vftIndex, F* function) {
		RTTIScanner::RTTI* pClass = RTTIScanner::getClassRTTI(className);
		if (!pClass) {
			Logger::log(Logger::ERR, "RTTIScanner failed to find class with name %s", className);
			return; 
		}
		Logger::log(Logger::DEBUG, "Hooking %s virtual function table at index %d", className, vftIndex);
	...
	virtual ~VFTHookTemplate() {
		...
		Logger::log(Logger::DEBUG, "Unhooking virtual function table at %p", (void*)topHook->hookData.fnHooked);
