#ifndef UNIHOOK_CALLHOOK_H
#define UNIHOOK_CALLHOOK_H
...
template <typename HookType> class CallHookTemplate : public CallHookBase {
public:
	...
	virtual ~CallHookTemplate() {
		...
		if (!hook) {
			Logger::log(Logger::ERR, "Unable to unhook call, it has no associated memory");
			return;
		}
		else {
			Logger::log(Logger::DEBUG, "Unhooking call at %p", reinterpret_cast<void*>(hook->hookData.previous));
		}
...

		{
			...
			if (hook->hookData.magic == prevHook->hookData.magic) {
				...
				try {
					...
					Logger::log(Logger::ERR, "Unable to initialize Zydis instruction decoder");
				}
			}
			Logger::log(Logger::DEBUG, "Successfully unhooked call at %p", reinterpret_cast<void*>(prevHook));
		}
...
private:
	template <typename F> void hook(void* callAddress, F* function) {
		Logger::log(Logger::DEBUG, "Hooking call at %p - allocating hook memory", callAddress);
		// Allocate executable memory for the hook
		void* allocationBase = moduleAlloc(sizeof(HookType));
		if (!allocationBase) {
			Logger::log(Logger::ERR, "Failed to hook call at %p - unable to allocate memory", callAddress);
			return;
		}
		...
		Logger::log(Logger::DEBUG, "Successfully hooked call at %p", callAddress);
	}
...
namespace CallHook {
	...
	public:
		CallMap() {
			...
			if (auto sections = PEParser::getSectionsWithName(".pdata")) {
				...
				Logger::log(Logger::DEBUG, "Found a total of %d functions and %d calls in %fs", rawCalls.size(), callCount, static_cast<float>(perfCounter2 - perfCounter1) / static_cast<float>(perfFrequency));
				...
				if (callCount != 0) {
					Logger::log(Logger::WARN, "Total count of calls found did not match the expected amount. Expected call count %d off by %d. Not all calls have been added to the call map", this->underlying.capacity(), std::abs(callCount));
				}
			}
			else {
				Logger::log(Logger::ERR, "Unable to locate .text section of the module. Make sure CallHook::initialize has been called");
			}
		}
	...
	static bool initialize(PEParser::ProcessInfo* pInfo = nullptr) {
		...
		if (!parser.parse(pInfo)) {
			...
			Logger::log(Logger::ERR, "Failed to parse PE headers of %s", _splitpath_s(modulePath.get(), NULL, 0, NULL, 0, moduleNameShort.get(), MAX_PATH, NULL, 0)
				|| !strnlen_s(modulePath.get(), MAX_PATH) ? "unkModule" : moduleNameShort.get());
			return false;
		}
		else {
			Logger::log(Logger::DEBUG, "Successfully initialized CallHook");
			return true;
		}
	}

	using FunctionHooks = std::vector<CallHookBase*>;
	...
	template <typename T, typename F> static FunctionHooks hookFunction(std::vector<void*> calls, F* function) {
		FunctionHooks hooks{};
		for (auto call : calls) {
	...
	template <typename T> static void unhookFunction(FunctionHooks& hooks) {
		for (auto hook : hooks) {
			delete hook;
		}
	}
