	...
	public:
		...
		bool addSection(Section* section) {
			...
			if (iter != this->sectionMap.end()) {
				...
				auto& newSection = this->sectionMap.emplace(std::make_pair(section->name, PESections())).first->second;
				newSection.emplace_back(std::make_unique<Section>());
				newSection.back().reset(section);
			}
