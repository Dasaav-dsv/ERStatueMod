...
namespace Pattern16 {
	namespace Impl {
		...
		PATTERN16_FORCE_INLINE auto _pext_u32_BMI_NONE(uint32_t a, uint32_t mask) {
			auto result = 0u;
			auto nmask = ~mask;
			while (mask) {
		...
		PATTERN16_FORCE_INLINE auto _pext_u32_BMI1(uint32_t a, uint32_t mask) {
			auto result = 0u;
			auto nmask = ~mask;
			while (mask) {
				auto start = _lzcnt_u32(mask);
		...
		PATTERN16_FORCE_INLINE auto _pdep_u32_BMI_NONE(uint32_t a, uint32_t mask) {
			auto result = 0u;
			auto nmask = ~mask;
			while (mask) {
