#ifndef PATTERN16_SCANNERS_BASE_H
#define PATTERN16_SCANNERS_BASE_H
...
namespace Pattern16 {
	namespace Impl {
		PATTERN16_NO_INLINE static const void* scanRegion(const void* regionStart, const void* regionEnd, const SplitSignatureU8& signature) {
		...
		PATTERN16_NO_INLINE static SplitSignatureU8 processSignatureString(const std::string& signature) {
			...
			for (auto chr : signature) {
				if (chr >= 0) {
					if (chr == ' ');
						...
						auto val = hexLookup[static_cast<int>(chr)];
						if (bit < 0) {
							if ((++counter &= 1)) {
								sig.push_back(0);
								mask.push_back(0);
		...
		template <typename T>
		PATTERN16_NO_INLINE static auto processSignatureBytes(SplitSignatureU8 signature) {
		...
		PATTERN16_NO_INLINE static auto getSigStartPos(const SplitSignatureU8& signature, const Frequencies16& cache) {
		...
		PATTERN16_NO_INLINE static const void* scanT(const void* regionStart, size_t regionSize, SplitSignatureU8& signature, const Frequencies16& frequencies) {
		...
		PATTERN16_NO_INLINE static const void* scan(const void* regionStart, size_t regionSize, SplitSignatureU8& signature, const Frequencies16& frequencies) {
			std::array<int, 4> cpuInfo;
			PATTERN16_CPUID_LEAF7(cpuInfo);
